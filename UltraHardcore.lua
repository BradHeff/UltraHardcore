...
UltraHardcore:SetScript('OnEvent', function(self, event, unit)
    if event == 'PLAYER_ENTERING_WORLD' or event == 'ADDON_LOADED' then
        ...
        -- Initialize ElvUI integration if available
        if _G.UltraHardcore_InitializeElvUIIntegration then
            _G.UltraHardcore_InitializeElvUIIntegration()
        end

        -- Create game menu button
        if _G.UltraHardcore_CreateMenuButton then
            _G.UltraHardcore_CreateMenuButton()
        end
        ...
        SetEnemyNameplateDisplay(GLOBAL_SETTINGS.hideEnemyNameplates or false)
