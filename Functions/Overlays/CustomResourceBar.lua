...
-- Position resource bar where the player frame would normally be (much higher up)
resourceBar:SetPoint('BOTTOMLEFT', UIParent, 'BOTTOMLEFT', 60, 270)
...
local POWER_COLORS = {
    ...
    RAGE = {1, 0, 0}, -- Red
    MANA = {0, 0, 1}, -- Blue
    CHI = {0, 1, 0.59}, -- Light Green (MoP Monk resource)
    FOCUS = {1, 0.5, 0.25}, -- Orange (Hunter resource)
...
-- Power type mappings for MoP Classic
...
local function UpdateResourcePoints()
    ...
    local powerIndex = POWER_TYPE_MAP[powerType] or 0
    local value = UnitPower('player', powerIndex)
    local maxValue = UnitPowerMax('player', powerIndex)

    resourceBar:SetMinMaxValues(0, maxValue)
    resourceBar:SetValue(value)

    -- Add fallback color in case powerType is not found
